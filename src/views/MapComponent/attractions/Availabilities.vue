<template>
  <div>
    ...
    >
      ...
      >
        <div class="pr-2">
          <p class="text-2xl text-main font-semibold text-slate-800 truncate">
            Availabilities
          </p>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-6">
        ...
        <!-- Form And Details -->
        <div
          class="sm:col-span-2 sm:h-[70vh] overflow-y-auto pb-32 px-0 sm:px-4"
        >
          <!-- Success Message -->
          <div
            v-if="showSuccess"
...
const updateCheckinDate = () => {
  if (
    ...
    formData.value.checkin_date = `${dateFormatData.value.year}-${String(
      dateFormatData.value.month,
    ).padStart(2, "0")}-${String(dateFormatData.value.day).padStart(2, "0")}`;
  }
};
